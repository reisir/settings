...
$Variable.Width=if($Variable.Width -match '^\s*(\d+\.?\d*?)\s*$'){$matches[1]}else{300}
$Variable.MinValue=if($Variable.MinValue -match '^\s*(\d+\.?\d*?)\s*$'){$matches[1]}else{0}
$Variable.MaxValue=if($Variable.MaxValue -match '^\s*(\d+\.?\d*?)\s*$'){$matches[1]}else{100}
...
MouseScrollUpAction=[!SetVariable $($Variable.Key) "$(&($doubler) -string "(Clamp([#$($Variable.Key)]+$($scrollHash.Increment), $($Variable.MinValue), $($Variable.MaxValue)))"-precision $Variable.Precision)"][!WriteKeyValue Variables $($Variable.Key) "[#$($Variable.Key)]" "$SettingsFile"][!SetOption SliderBlob$($Variable.Index) X "([#s_LeftPanelW]+[#s_VariableXPadding]+10+$($Variable.Width)*([#$($Variable.Key)]-($($Variable.MinValue)))/($($Variable.MaxValue - $Variable.MinValue)))"][!UpdateMeterGroup Sliders$($Variable.Index)][!Redraw][#s_OnChangeAction]
...
Measure=Plugin
Plugin=Mouse
LeftMouseDragAction=[!SetOption SliderBlob$($Variable.Index) X "(Clamp(`$MouseX$,[#s_LeftPanelW]+[#s_VariableXPadding]+10,([#s_LeftPanelW]+[#s_VariableXPadding]+10+$($Variable.Width))))"][!UpdateMeter SliderBlob$($Variable.Index)][!UpdateMeasure SliderCalc$($Variable.Index)]
...
MinValue=([#s_LeftPanelW]+[#s_VariableXPadding]+10)
MaxValue=([#s_LeftPanelW]+[#s_VariableXPadding]+10+$($Variable.Width))
...
Meter=Shape
X=([#s_LeftPanelW]+[#s_VariableXPadding]+10+$($Variable.Width)*([#$($Variable.Key)]-($($Variable.MinValue)))/($($Variable.MaxValue)-($($Variable.MinValue))))
Y=3r
